...
numUsefulOFDM = 12; % num time symbols
numSCS = 12;% num subcarriers
K = carrier.NSizeGrid * numSCS;
numPilots = [1 2 4 6 12];
numPilotLevel = length(numPilots);

%% Hold data separately for each DU in $$_SingleDU$$ data structures
NMSE_Hmk_MP_SNR = zeros(numPilotLevel,1);
NMSE_Hmk_MP_NoPilot_SNR = zeros(numPilotLevel,1);
NMSE_Hmk_DFT_SNR = zeros(numPilotLevel,1);
NMSE_Hmk_LMMSE_SNR = zeros(numPilotLevel,1);

BER_MP_Pilot = zeros(numPilotLevel,1);
BER_MP_NoPilot = zeros(numPilotLevel,1);
BER_DFT = zeros(numPilotLevel,1);
BER_MMSE = zeros(numPilotLevel,1);
BER_True = zeros(numPilotLevel,1);

totalNumChannels = 50; % Monte-Carlo trials
tolFloating = 1e-2;
...
for channelIdx = 1:totalNumChannels
    ...
    noisePower = noiseAmp^2;
    rxGridMultiDUSNR = sigGridMultiDU + noiseGridMultiDUSNR;
    for pLenIdx = 1:numPilotLevel
        numPilotSyms = numPilots(pLenIdx);
        ...
        for DUIdx = 1:simParams.NumDU
            ...
            rxGrid = sigGridMultiDU(:,:,antennaRange);
         ...
         for DUIdx = 1:simParams.NumDU
             ...
             Xpilot = txGrid(freqIdx,1:numPilotSyms);
             Ypilot = reshape(rxGrid(freqIdx,1:numPilotSyms,:),[],numPilotSyms,simParams.NumRx);
             ...
             Hest_MP_NoPilot(antennaRange) = Hest_MP_NoPilot(antennaRange) + (mag_mp_nopilot./numPilotSyms);
         ...
         NMSE_Hmk_MP_SNR(pLenIdx) = NMSE_Hmk_MP_SNR(pLenIdx) + tmp_NMSE_MP;
         NMSE_Hmk_MP_NoPilot_SNR(pLenIdx) = NMSE_Hmk_MP_NoPilot_SNR(pLenIdx) + tmp_NMSE_MP_NoPilot;
         NMSE_Hmk_DFT_SNR(pLenIdx) = NMSE_Hmk_DFT_SNR(pLenIdx) + tmp_NMSE_DFT;
         NMSE_Hmk_LMMSE_SNR(pLenIdx) = NMSE_Hmk_LMMSE_SNR(pLenIdx) + tmp_NMSE_LMMSE;
        
         BER_MP_Pilot(pLenIdx) = BER_MP_Pilot(pLenIdx) + tmp_BER_MP;
         BER_MP_NoPilot(pLenIdx) = BER_MP_NoPilot(pLenIdx) + tmp_BER_MP_NoPilot;
         BER_DFT(pLenIdx) = BER_DFT(pLenIdx) + tmp_BER_DFT;
         BER_MMSE(pLenIdx) = BER_MMSE(pLenIdx) + tmp_BER_LMMSE;
         BER_True(pLenIdx) = BER_True(pLenIdx) + tmp_BER_True;
    end
end

NMSE_Hmk_MP_SNR = NMSE_Hmk_MP_SNR ./ totalNumChannels;
NMSE_Hmk_MP_NoPilot_SNR = NMSE_Hmk_MP_NoPilot_SNR ./ totalNumChannels;
NMSE_Hmk_DFT_SNR = NMSE_Hmk_DFT_SNR ./ totalNumChannels;
NMSE_Hmk_LMMSE_SNR = NMSE_Hmk_LMMSE_SNR ./ totalNumChannels;
...
fig1=figure;
semilogy(numPilots,NMSE_Hmk_MP_SNR,'-o','DisplayName','Hmk Matrix Pencil (Pilot-Training)');
hold on;
semilogy(numPilots,NMSE_Hmk_MP_NoPilot_SNR,'-o','DisplayName','Hmk Matrix Pencil (Zero-Pilot)');
semilogy(numPilots,NMSE_Hmk_DFT_SNR,'-^','DisplayName','Hmk DFT');
semilogy(numPilots,NMSE_Hmk_LMMSE_SNR,'-*','DisplayName','Hmk MMSE');
grid on
xlabel('Number of Training Pilot Symbols')
...
legend show
hold off
pngfile=sprintf('NMSE_Pilot_servR%d_%dPaths_j%s',serveRadius,simParams.NumPaths,job);
print(fig1,pngfile,'-dpng')


fig2=figure;
semilogy(numPilots,BER_MP_Pilot,'-o','DisplayName','BER Matrix-Pencil (Pilot-Training)');
hold on
semilogy(numPilots,BER_MP_NoPilot,'-o','DisplayName','BER Matrix-Pencil (Zero-Pilot)');
semilogy(numPilots,BER_DFT,'-^','DisplayName','BER DFT');
semilogy(numPilots,BER_MMSE,'-*','DisplayName','BER Linear-MMSE');
grid on
xlabel('Number of Training Pilot Symbols')
ylabel('BER')
...
legend show     
hold off
pngfile=sprintf('BER_Pilot_servR%d_%dPaths_j%s',serveRadius,simParams.NumPaths,job);
